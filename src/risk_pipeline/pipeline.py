...
def pick_latest_with_lookback_and_basis(df: pd.DataFrame, lookback: int) -> Tuple[pd.DataFrame, Dict[str,str]]:
    ...
    chosen_rows = []; basis_map: Dict[str,str] = {}
    for iso, y_latest in latest_year.items():
        sub = df[df["iso3"]==iso]
        used_row = None; used_basis = None
        ...
        if used_row is not None:
            chosen_rows.append(used_row); basis_map[iso] = used_basis
    ...
    for level in order:
        ...
        if level=="region":
            ...
            for iso in values[mask].index:
                ...
                v = grp.get(r, np.nan)
                if pd.notna(v): values.loc[iso]=v; basis.loc[iso]="region"
        elif level=="income":
            ...
            for iso in values[mask].index:
                ...
                v = grp.get(g, np.nan)
                if pd.notna(v): values.loc[iso]=v; basis.loc[iso]="income"
        elif level=="global":
            v = df_values["value"].median() if not df_values.empty else np.nan
            if pd.notna(v): values[mask]=v; basis[mask]="global"
    return values, basis
...
def normalize(values: pd.Series, spec: Dict[str,Any]) -> pd.Series:
    ...
    if method=="invert_0_100": return (100 - v).clip(0,100)
    if method=="wgi":
        r = ((2.5 - v) / 5.0) * 100.0
    ...
    if method=="linear_minmax":
        ...
        r = ((v - mn) / (mx - mn)) * 100.0
        if not higher_is_riskier: r = 100.0 - r
        return np.clip(r, 0, 100)
    if method=="log_cap":
...
def run_pipeline():
    ...
    for ind in cfg["indicators"]:
        ...
        values_full, basis_full = impute_with_basis(
            ser_used.reindex(iso_list), used, countries,
            cfg["defaults"]["imputation"]["order"], basis0
        )
    ...
    # ND-GAIN Fallback (synthetic) falls leer
    if "nd_gain" in raw_values:
        all_nan = raw_values["nd_gain"].isna().all() if len(raw_values["nd_gain"])>0 else True
        if all_nan:
            env_ids = [ind["id"] for ind in cfg["indicators"] if ind["pillar"]=="environment" and ind["id"]!="nd_gain"]
            if env_ids:
                env_norm_df = pd.DataFrame({iid: norm_values[iid] for iid in env_ids})
                env_risk_avg = env_norm_df.mean(axis=1, skipna=True)
                synthetic_value = 100.0 - env_risk_avg
                ...
                for ind in cfg["indicators"]:
                    if ind["id"]=="nd_gain":
                        ...
                        break
                if nd_spec is None: nd_spec = cfg["defaults"]["normalization"]
    ...
    for _, row in df_wide.iterrows():
        ...
        for iid in raw_values.keys():
            recs.append({
                "iso3": iso, "kind": "indicator", "id": iid,
                "raw": row.get(f"raw__{iid}"), "norm": row.get(f"norm__{iid}"),
                "basis": row.get(f"basis__{iid}"), "year": row.get(f"year__{iid}"),
            })
    ...
    meta = {
        ...
        "notes": [
            ...
            "ND-GAIN: Falls Quelle leer, transparenter synthetischer Ersatz aus anderen Environment-Indikatoren (basis=synthetic)."
        ]
    }
